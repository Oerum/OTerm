...
<template>
  <div class="relative flex min-h-0 flex-1 flex-col bg-[var(--oterm-bg)] text-[var(--oterm-text)]">
    ...
    >
      ...
      <button
        type="button"
        class="rounded-md border border-[var(--oterm-danger)]/40 px-2 py-1 text-xs text-[var(--oterm-danger)] hover:bg-[var(--oterm-danger)]/10 disabled:opacity-50"
      ...
      <button
        type="button"
        class="rounded-md border border-[var(--oterm-border)] px-2 py-1 text-xs hover:bg-white/5 disabled:opacity-50"
    ...
    >
      <div>
        <p class="font-medium text-[var(--oterm-text)]">Docker is unavailable</p>
    ...
    <template v-else>
      <div class="oterm-scroll min-h-0 flex-1 overflow-auto p-4">
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <div class="rounded-lg border border-[var(--oterm-border)] bg-[var(--oterm-panel)] p-3">
            <div class="text-xs uppercase text-[var(--oterm-muted)]">Containers</div>
            <div class="mt-1 text-2xl font-medium">{{ summary.containers.length }}</div>
            <div class="mt-1 text-xs text-[var(--oterm-muted)]">
              {{ activeContainers.length }} active · {{ pausedContainers.length }} paused ·
              {{ stoppedContainers.length }} stopped
            </div>
          </div>
          <div class="rounded-lg border border-[var(--oterm-border)] bg-[var(--oterm-panel)] p-3">
            <div class="text-xs uppercase text-[var(--oterm-muted)]">Images</div>
          ...
          <div class="rounded-lg border border-[var(--oterm-border)] bg-[var(--oterm-panel)] p-3">
            <div class="text-xs uppercase text-[var(--oterm-muted)]">Volumes</div>
            <div class="mt-1 text-2xl font-medium">{{ summary.volumes.length }}</div>
            <div class="mt-1 text-xs text-[var(--oterm-muted)]">Persistent Docker storage</div>
          </div>
          <div class="rounded-lg border border-[var(--oterm-border)] bg-[var(--oterm-panel)] p-3">
            <div class="text-xs uppercase text-[var(--oterm-muted)]">Networks</div>
            <div class="mt-1 text-2xl font-medium">{{ summary.networks.length }}</div>
            <div class="mt-1 text-xs text-[var(--oterm-muted)]">Bridge, host, and custom networks</div>
          </div>
        </div>

        <section class="mt-5 space-y-3">
          <div class="flex items-center gap-2">
            <h3 class="text-xs font-semibold uppercase tracking-wide text-[var(--oterm-muted)]">
              Containers
            </h3>
            <div class="flex-1" />
            <button
              ...
              type="button"
              class="rounded border border-[var(--oterm-danger)]/40 px-2 py-0.5 text-[10px] text-[var(--oterm-danger)] hover:bg-[var(--oterm-danger)]/10 disabled:opacity-50"
              :disabled="busy"
              @click="
                pruneUnused(
                  'containers',
          ...
          <div v-if="summary.containers.length === 0" class="text-sm text-[var(--oterm-muted)]">
            No containers found.
          </div>
          <div v-for="group in containerGroups" :key="group.title">
            <h4 v-if="group.items.length" class="mb-2 text-xs font-medium text-[var(--oterm-muted)]">
              {{ group.title }}
            </h4>
            <div class="space-y-1.5">
              ...
              >
                <div class="min-w-0">
                  ...
                  <div class="mt-0.5 font-mono text-[10px] text-[var(--oterm-muted)]">
                    {{ container.id }}
                  </div>
                </div>
                <div class="min-w-0">
                  <div class="truncate text-xs text-[var(--oterm-muted)]">{{ container.image }}</div>
                  <div class="mt-0.5 truncate text-[10px] text-[var(--oterm-faint)]">
                ...
                <div class="min-w-0">
                  ...
                  <div class="mt-1 truncate text-[10px] text-[var(--oterm-muted)]">
                    {{ container.status }}
                  </div>
                </div>
                <div class="flex flex-wrap items-center justify-end gap-1" @click.stop>
                  ...
                  <button
                    v-if="container.state !== 'running'"
                    type="button"
                    class="rounded border border-[var(--oterm-border)] px-2 py-0.5 text-xs hover:bg-white/5 disabled:opacity-50"
                    :disabled="busy"
                  ...
                  <button
                    ...
                    type="button"
                    class="rounded border border-[var(--oterm-danger)]/40 px-2 py-0.5 text-xs text-[var(--oterm-danger)] hover:bg-[var(--oterm-danger)]/10 disabled:opacity-50"
                    :disabled="busy"
        ...
        <section class="mt-6 space-y-3">
          ...
          <div v-if="summary.images.length === 0" class="text-sm text-[var(--oterm-muted)]">
            No images found.
          </div>
          <div v-for="group in imageGroups" :key="group.title">
            ...
            <div class="space-y-1.5">
              <div
                ...
                class="grid gap-3 rounded-lg border border-[var(--oterm-border)] bg-[var(--oterm-panel)] px-3 py-2 text-sm lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_auto]"
              >
                <div class="min-w-0">
                  <div class="truncate font-medium">
                    {{ image.repository
                    }}<span class="text-[var(--oterm-muted)]">:{{ image.tag }}</span>
                  </div>
                  <div class="mt-0.5 font-mono text-[10px] text-[var(--oterm-muted)]">
                    {{ image.id }}
                  </div>
                </div>
                <div class="text-xs text-[var(--oterm-muted)]">{{ image.size }}</div>
                <div class="truncate text-xs text-[var(--oterm-muted)]">{{ image.createdSince }}</div>
                <div class="flex justify-end">
                  <button
                    ...
                    :class="
                      image.inUse
                        ? 'border-[var(--oterm-border)] text-[var(--oterm-muted)]'
                        : 'border-[var(--oterm-danger)]/40 text-[var(--oterm-danger)] hover:bg-[var(--oterm-danger)]/10'
                    "
        ...
        <section class="mt-6 grid gap-4 xl:grid-cols-2">
          <div>
            ...
            <div v-if="summary.volumes.length === 0" class="text-sm text-[var(--oterm-muted)]">
              No volumes found.
            </div>
            <div class="space-y-1.5">
              <div
                ...
                :key="volume.name"
                class="flex items-center gap-3 rounded-lg border border-[var(--oterm-border)] bg-[var(--oterm-panel)] px-3 py-2 text-sm"
              >
                <div class="min-w-0 flex-1">
                  <div class="truncate font-medium">{{ volume.name }}</div>
                  <div class="mt-0.5 text-[10px] text-[var(--oterm-muted)]">
          ...
          <div>
            ...
            <div v-if="summary.networks.length === 0" class="text-sm text-[var(--oterm-muted)]">
              No networks found.
            </div>
            <div class="space-y-1.5">
              <div
      ...
      <section
        ...
        class="flex h-56 shrink-0 flex-col border-t border-[var(--oterm-border)] bg-[var(--oterm-panel)]"
      >
        <div class="flex items-center gap-2 border-b border-[var(--oterm-border)] px-3 py-2">
          ...
          <button
            v-if="selectedIsLive"
            type="button"
            class="rounded border border-[var(--oterm-accent)]/40 px-2 py-0.5 text-[10px] text-[var(--oterm-accent)] hover:bg-[var(--oterm-accent)]/10"
          ...
          <button
            type="button"
            class="rounded border border-[var(--oterm-border)] px-2 py-0.5 text-[10px] hover:bg-white/5 disabled:opacity-50"
          ...
          <button
            type="button"
            class="rounded border border-[var(--oterm-border)] px-2 py-0.5 text-[10px] hover:bg-white/5"
        ...
        <pre
          class="oterm-scroll m-0 min-h-0 flex-1 overflow-auto p-3 font-mono text-[11px] leading-relaxed text-[var(--oterm-text)]"
