...
export function listDirectory(path?: string, includeHidden = false): Promise<FsEntry[]> {
  return invoke<FsEntry[]>("fs_list_directory", {
    path: path ?? null,
    includeHidden,
  });
}
...
export function searchFiles(
  ...
  return invoke<FsEntry[]>("fs_search_files", { root: root ?? null, query, includeHidden });
}
