...
## 2026-09-11 - Path Traversal in SFTP File Download
**Vulnerability:** Path traversal in `onRemoteOpen` when downloading remote files via SFTP to a local temp directory. The untrusted `entry.name` from the remote server was used directly in `joinPath(localTempDir, entry.name)`.
**Learning:** File names obtained from untrusted or remote sources (like SFTP servers) must be validated before being used in local file system paths to prevent directory traversal and arbitrary file overwrite.
**Prevention:** Always validate remote file names by checking for directory separators (`/`, `\`) or relative navigation components (`..`, `.`) before using them in local path construction.
