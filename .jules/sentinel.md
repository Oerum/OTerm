...
## 2026-08-31 - SSH Option Injection via Username/Hostname
**Vulnerability:** In `src/lib/sshOpenSshArgs.ts`, the destination string (`${endpoint.username}@${endpoint.host}`) was passed directly as an argument. If a malicious or malformed username began with a hyphen (e.g., `-oProxyCommand=calc`), OpenSSH would interpret it as a command-line option rather than the destination, potentially leading to arbitrary command execution.
**Learning:** Even if individual arguments are properly shell-quoted, command-line utilities (like `ssh`, `git`, `rm`, `mosh`) parse their own arguments. If an argument starts with a hyphen, it may be treated as an option instead of a positional argument (like a filename or destination). Furthermore, options like `-t` must precede `--`; placing options after `--` causes the option parser to treat them as positional arguments (such as the remote command to execute).
**Prevention:** Always use `--` to explicitly signal the end of command-line options before passing untrusted or dynamic data that serves as a positional argument. Ensure all CLI options (like `-t`) are placed before `--`. For `mosh`, pass `--` before the destination and ensure `--ssh` contains only client options without the target host.

## 2026-09-02 - Command Injection in launchAgent via pendingTerminalCommands
**Vulnerability:** The `launchAgent` function in `src/App.vue` was passing the agent ID or prefix directly into `pendingTerminalCommands.set` without escaping it for the target shell (`quoteForShell`). While currently only hardcoded agent prefixes exist in `src/lib/terminalAgentMode.ts` (making it an enhancement rather than a critical exploit vector right now), it established a vulnerable pattern that could lead to command injection if agent prefixes become user-configurable or contain shell metacharacters in the future.
**Learning:** Any dynamic string interpolated into a command that is later sent to a terminal session (e.g. via `pendingTerminalCommands.set`) must be securely escaped according to the target shell. Relying on the currently benign nature of internal configuration is fragile and violates defense-in-depth principles.
**Prevention:** Always use the `quoteForShell(value, shellId)` utility from `src/lib/shellQuote.ts` whenever concatenating non-constant strings into terminal payload commands, regardless of the string's current source.
