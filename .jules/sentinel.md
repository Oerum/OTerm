...
## 2023-10-27 - [HIGH] Command Injection in Terminal Navigation
...
**Learning:** Shell commands constructed by string interpolation must be carefully sanitized based on the target shell. What's safe for `pwsh` is not necessarily safe for `cmd` or `bash`.
**Prevention:** Always use `shellQuote` for POSIX shells, and shell-specific escaping (like stripping quotes for `cmd`, or using `-LiteralPath` for PowerShell) when passing user-controlled data to terminal inputs.
