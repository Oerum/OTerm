...
**Vulnerability:** In `buildOpenSshArgs` (`src/lib/sshOpenSshArgs.ts`), the destination string (`${endpoint.username}@${endpoint.host}`) was passed directly as an argument. If a malicious or malformed username began with a hyphen (e.g., `-oProxyCommand=calc`), OpenSSH would interpret it as a command-line option rather than the destination, potentially leading to arbitrary command execution.
**Learning:** Even if individual arguments are properly shell-quoted, command-line utilities (like `ssh`, `git`, `rm`) parse their own arguments. If an argument starts with a hyphen, it may be treated as an option instead of a positional argument (like a filename or destination).
**Prevention:** Always use `--` to explicitly signal the end of command-line options before passing untrusted or dynamic data that serves as a positional argument (e.g., `args.push("--", destination)`).
